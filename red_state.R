...
#Red State defined by having GOP governor... not the presidential election
cities_in_red_state = c("Dallas", "Houston", "Miami", "Atlanta", "Phoenix",
                        "Tampa")
...
for (city in MSA_list){
  ...
  #### create variable for Red State ####
  data_MSA_summer_naics$state_gov = if_else(city %in% cities_in_red_state,
                                            "red",
                                            "blue")
...
data_all_summer$red_state = factor(data_all_summer$red_state)
...
#### create variables for red x year ####
data_all_summer$red_2020 = factor(if_else(data_all_summer$year==2020 &
                                          data_all_summer$state_gov=="red",
                                          1,
                                          0))

data_all_summer$red_2021 = factor(if_else(data_all_summer$year==2021 &
                                          data_all_summer$state_gov=="red",
                                          1,
                                          0))

#### create variables for red x SES x year ####
data_all_summer$red_above_2020 = factor(if_else(data_all_summer$year==2020 &
                                                data_all_summer$SES=="above" &
                                                data_all_summer$state_gov=="red",
                                                1,
                                                0))

data_all_summer$red_above_2021 = factor(if_else(data_all_summer$year==2021 &
                                                data_all_summer$SES=="above" &
                                                data_all_summer$state_gov=="red",
                                                1,
                                                0))
...
fit_1 = lm(log(visitors) ~ year + above_median_2020 + above_median_2021 +
                           red_2020 + red_2021 + red_above_2020 +
                           red_above_2021 + month + MSA_SES,
